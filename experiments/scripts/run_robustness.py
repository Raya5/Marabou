...
experiments/results/robustness/network/<smoke|full|subset>/point_<db_idx>/<baseline|incremental>/summary.json

Fixed configuration is hardcoded below.
"""
...
class RobustnessConfig:
    ...
    onnx_path: str = "experiments/data/mnist4.onnx"
    eps_min: float = 0.0
    eps_max: float = 0.5
    precision: float = 1e-3
    # timeout_per_query_sec: float = 120.0
    # all_timeout_sec: float = 1200.0
    timeout_per_query_sec: float = 300.0
    all_timeout_sec: float = 3600.0
    margin: float = 0.0
    input_min: float = 0.0
    input_max: float = 1.0
...
def solve_at_epsilon(
    *,
    ...
    if incremental:
        ...
        # For robustness-radius search, every epsilon query may be useful
        # for future smaller-epsilon queries, so we record conflicts for all.
...
def main():
    ...
    p.add_argument(
        ...
        type=int,
        required=True,
        help="MNIST test-set index in [0, 999]",
    )
    p.add_argument(
        "--smoke",
        action="store_true",
    ...
    onnx_name = Path(cfg.onnx_path).stem
    out_root = (
        Path("experiments")
        / "results"
        / "robustness"
